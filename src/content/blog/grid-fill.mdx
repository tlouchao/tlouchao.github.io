...
From testing out the edge loop selection on cylinder primitives in Maya, I found that the edge/vertex component indices were returned in ascending order, 
...
Adding some number, or the **offset**, to both the starting endpoint `i` and opposite endpoint `j`, would also rotate the edge by that number. 
I needed the **modulo** operator to help resolve to the actual indices, in case the adjusted indices wrapped around 
to the other end of the list of selected vertices:
...
To create the edges on either side, I concluded that there were two ways to get the adjacent endpoints:
...
And, the existing edges would need to be subdivided to provide vertices for future `polySplit` / `polyConnectComponents` operations to connect to. 
...
I offset the outermost "row" vertices by 2, in order to skip over the corners:
...
Once again, I relied on [Qt Designer](https://build-system.fman.io/qt-designer-download) to prototype the GUI to interact with.
