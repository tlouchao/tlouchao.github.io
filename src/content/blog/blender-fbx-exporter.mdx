...
```
With this information, I understood where to begin for the scale transform option on the output FBX, at the very least.
...
I attached one [`StringProperty`](https://docs.blender.org/api/current/bpy.types.StringProperty.html#stringproperty-property) named `project_path` to the current
...
self.layout.prop(context.scene, "project_path", placeholder=placeholder))
```

And replaced the hardcoded filepath in my operator with the new value:

``` python
filepath = os.path.join(context.scene.project_path, stem + '.fbx')
```

#### The File Select Operator ####

Still, I needed some way to open a file browser and assign the resulting filepath to the `project_path`, to make the Blender Add-on *much* easier to use. 
...
class ProjectPathOperator(bpy.types.Operator):
    ...
    bl_idname = 'export_scene.project_path'
    bl_label = "Add"
    ...
    def execute(self, context):
        context.scene.project_path = self.directory
        return {'FINISHED'}
```
...
``` python
self.layout.operator('export_scene.project_path')
```
